/**
 * Script to update roundtable prompts with voice characteristic guidance
 *
 * Run with: npx tsx scripts/update-roundtable-prompts-voice.ts
 */
...
async function main() {
  console.log('🎭 Updating roundtable prompts with voice characteristics...\n');
  ...
  const conversationTurnPrompt = await prisma.prompt.upsert({
    ...
    create: {
      ...
      description: 'Prompt for persona to respond during ongoing conversation with voice characteristics',
  ...
  // Create new version with voice characteristics
...
- Use other jurors' names when referring to their points`,
  ...
  console.log('✅ Updated conversation turn prompt with voice characteristics (v3.0.0)');

  // Update initial reaction prompt with voice characteristics
  const initialReactionPrompt = await prisma.prompt.upsert({
    ...
    create: {
      ...
      description: 'Prompt for persona first reaction to an argument with voice characteristics',
      category: 'focus-group'
    }
  });
...
{{/if}}
{{#if responseTendency}}
- You typically give {{responseTendency}} responses
{{/if}}
...
- Use your characteristic speech patterns and vocabulary level`,
  ...
  console.log('✅ Updated initial reaction prompt with voice characteristics (v3.0.0)');
